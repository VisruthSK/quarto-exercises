...
For grouped code cloze, put behavior options such as `instant`, `reset`, `reveal`, `lock`, and feedback text on the `.exercise`. The `{{blank ...}}` and `{{choose ...}}` markers should only describe the answer check itself: answers, matching, casing, whitespace handling, options, and shuffle.
