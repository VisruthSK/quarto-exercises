...
```markdown
The literal token is [yes\\|no|maybe|unknown]{.choose answer="yes|no"}.
```
...
Inside an `.exercise`, inline `.blank` and `.choose` controls inherit behavior from the parent exercise. Put `instant`, `reveal`, `lock`, and `reset` on the `.exercise`; individual controls should only describe their own answer check and feedback text.
...
In a grouped code cloze, the parent `.exercise` controls `reset`, `reveal`, `lock`, and feedback text. The code block and its markers can use `instant` and answer-checking attributes such as answers, matching, casing, whitespace handling, options, and shuffle.
...
Unsupported attributes warn during render in both HTML and non-HTML output.
````
