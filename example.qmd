...
Fill in the blanks directly inside code blocks. Syntax highlighting is fully preserved.
...
### In a Parent Exercise

::: {.exercise}
Complete the Python snippet.
...
::: {.exercise}
Complete the R snippet.

```{.code-cloze lang="r"}
x <- 1:10
result <- {{choose answer="mean" options="mean,median,sum,length"}}(x)
cat({{blank answer="result"}}, "\n")
```
:::
